...
class G29:
    ...
    def find_g29(self):
        for path in list_devices():
            ...
            if "G29" in dev.name or "Driving Force" in dev.name:
                print("NAME:", dev.name)
                print("EVENT:", dev.path)

                event_name = dev.path.split("/")[-1]
                input_sysfs = os.path.realpath(f"/sys/class/input/{event_name}/device")

                print("INPUT SYSFS:", input_sysfs)

                # extract USB ID (this is the key fix)
                usb_id = None
                for part in input_sysfs.split("/"):
                    if "046D:C24F" in part.upper():
                        usb_id = part
                        break
                ...
                # now match hidraw properly
                for h in os.listdir("/sys/class/hidraw"):
                    hid_sys = os.path.realpath(f"/sys/class/hidraw/{h}/device")

                    if usb_id and usb_id in hid_sys:
                        hidraw_path = f"/dev/{h}"
                        print("HIDRAW:", f"/dev/{h}")

                event_dev = dev
                try:
                    hid_dev = hid.Device(path=hidraw_path.encode()) if hidraw_path else None
                except Exception as e:
                    print(e)

                return hid_dev
        
        raise Exception("G29 not found")
